    ...
    h("div", { "data-keep-scroll": "settings", style: "flex:1;min-height:0;overflow:auto;padding:0 24px 24px" },
      h("div", { style: "max-width:760px" },
        ...
        vodLibOn ? settingsSection("VOD LIBRARY (EMBY / JELLYFIN)",
          ...
          settingRow({ title: "Include TV series", desc: "Also mirror series as Season/Episode .strm stubs under <path>/Series — browsable in Emby, and importable by Sonarr as a zero-storage catalog (monitor a show there to download it for real).", key: "vod.includeSeries", type: "toggle" }),
          s["vod.includeSeries"] ? settingRow({ title: "Refresh existing series", desc: "Re-check every mirrored series on each run so newly-aired episodes appear automatically. Leave on — turning this off freezes the catalog.", key: "vod.refreshExisting", type: "toggle" }) : null,
          settingRow({ title: "Sonarr indexer (Torznab)", desc: "Expose the series catalog to Sonarr at /torznab. Sonarr's own searches trigger live provider lookups and grabs write single .strm stubs — no bulk mirroring needed for monitored shows.", key: "vod.indexer.enabled", type: "toggle" }),
          s["vod.indexer.enabled"] ? settingRow({ title: "Indexer API key", desc: "The apikey Sonarr uses for /torznab/api (auto-generated). Indexer URL: http://<phospharr>:7777/torznab", key: "vod.indexer.apiKey", type: "text" }) : null,
          s["vod.indexer.enabled"] ? settingRow({ title: "Blackhole NZB folder", desc: "Sonarr's Usenet Blackhole “Nzb Folder” — where grabbed releases land.", key: "vod.indexer.blackholeWatchPath", type: "text" }) : null,
